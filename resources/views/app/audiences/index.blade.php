            ...
            <x-dashboard.section title="Create audience" description="Use brand scope for campaign lists, or account scope for shared contact pools.">
                <form method="POST" action="{{ route('app.audiences.store') }}" class="space-y-4">
                    ...
                    <label class="block">
                        ...
                        <input name="name" required class="mt-1 w-full rounded-md border border-line bg-white px-3 py-2 text-sm text-ink" placeholder="AI visibility newsletter list">
                    </label>
                    <textarea name="description" rows="3" class="w-full rounded-md border border-line bg-white px-3 py-2 text-sm text-ink" placeholder="Audience description"></textarea>
                ...
                @else
                    <div class="space-y-3">
                        @foreach ($audiences as $audience)
                            <a href="{{ route('app.audiences.show', $audience) }}" class="block rounded-md border border-line bg-panel p-4 transition hover:border-slate-300 hover:bg-white">
            ...
            <x-dashboard.section title="Create segment" description="Store rule definitions for later targeting; no activation runs from here yet.">
                <form method="POST" action="{{ route('app.segments.store') }}" class="space-y-4">
                    ...
                    <label class="block">
                        ...
                        <select name="audience_id" class="mt-1 w-full rounded-md border border-line bg-white px-3 py-2 text-sm text-ink">
                    ...
                    <label class="block">
                        <span class="text-xs font-semibold uppercase tracking-[0.12em] text-muted">Name</span>
                        <input name="name" required class="mt-1 w-full rounded-md border border-line bg-white px-3 py-2 text-sm text-ink" placeholder="Engaged operators">
                    </label>
                    <textarea name="description" rows="2" class="w-full rounded-md border border-line bg-white px-3 py-2 text-sm text-ink" placeholder="Segment description"></textarea>
                    <textarea name="rules_json" rows="4" class="w-full rounded-md border border-line bg-white px-3 py-2 font-mono text-xs text-ink" placeholder='{"field":"status","operator":"equals","value":"active"}'></textarea>
