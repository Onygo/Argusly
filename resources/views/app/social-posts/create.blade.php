        ...
        <x-ui.card class="mt-8 p-6">
            <form method="POST" action="{{ route('app.social-posts.store') }}" class="space-y-5">
                ...
                <label class="block">
                    ...
                    <textarea name="post_text" rows="8" class="mt-2 w-full rounded-md border border-line bg-white px-3 py-2 text-sm leading-6 text-ink" required>{{ old('post_text', $asset?->excerpt ?: $asset?->title) }}</textarea>
                    @error('post_text')
                        <span class="mt-2 block text-sm font-medium text-red-600">{{ $message }}</span>
                    @enderror
                </label>
                ...
                <div class="grid gap-4 sm:grid-cols-3">
                    ...
                    <label>
                        ...
                        <input name="market" value="{{ old('market', $brand->market) }}" class="mt-2 w-full rounded-md border border-line bg-white px-3 py-2 text-sm text-ink">
                    </label>
                </div>
