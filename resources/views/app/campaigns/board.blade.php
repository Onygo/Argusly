            ...
            <x-dashboard.section title="Create board item" description="Add work manually or point the item at an existing content asset, social post, task or recommendation.">
                <form method="POST" action="{{ route('app.campaigns.board.items.store', $campaign) }}" class="grid gap-4 lg:grid-cols-4">
                    ...
                    <label class="block">
                        ...
                        <input name="title" required class="mt-1 w-full rounded-md border border-line bg-white px-3 py-2 text-sm text-ink" placeholder="Draft launch article">
                    </label>
                    <label class="block">
                        <span class="text-xs font-semibold uppercase tracking-[0.12em] text-muted">Stage</span>
                        <select name="campaign_stage_id" class="mt-1 w-full rounded-md border border-line bg-white px-3 py-2 text-sm text-ink">
                    ...
                    <label class="block">
                        ...
                        <input name="due_at" type="datetime-local" class="mt-1 w-full rounded-md border border-line bg-white px-3 py-2 text-sm text-ink">
                    </label>
                    <label class="block">
                        <span class="text-xs font-semibold uppercase tracking-[0.12em] text-muted">Related type</span>
                        <select name="related_type" class="mt-1 w-full rounded-md border border-line bg-white px-3 py-2 text-sm text-ink">
                    ...
                    <label class="block">
                        ...
                        <input name="related_id" type="number" class="mt-1 w-full rounded-md border border-line bg-white px-3 py-2 text-sm text-ink" placeholder="Optional ID">
                    </label>
                    <label class="block">
                        <span class="text-xs font-semibold uppercase tracking-[0.12em] text-muted">Description</span>
                        <input name="description" class="mt-1 w-full rounded-md border border-line bg-white px-3 py-2 text-sm text-ink" placeholder="Short planning note">
                    </label>
                    ...
                    <section class="rounded-md border border-line bg-white">
                        ...
                        <div class="space-y-3 p-3">
                            @forelse ($stage->items as $item)
                                <article class="rounded-md border border-line bg-panel p-3">
                            ...
                            @empty
                                <div class="rounded-md border border-dashed border-line bg-panel p-4 text-sm text-muted">No items yet.</div>
                            @endforelse
                        </div>
                    </section>
                @endforeach
