<form method="GET" class="mb-4 grid gap-2 lg:grid-cols-6">
    ...
    <div class="lg:col-span-6 flex gap-2">
        ...
        <a href="{{ route('app.billing.index', ['tab' => 'ledger']) }}" class="rounded border border-border px-3 py-1.5 text-xs text-textSecondary">Reset</a>
    </div>
</form>
...
<div class="space-y-2">
        ...
        <button type="button" class="w-full rounded-md border border-border p-3 text-left hover:bg-surfaceSubtle/30" data-open-drawer data-drawer='@json($ledgerDrawer)'>
            <div class="flex items-center justify-between gap-3">
                <div>
                    ...
                    <p class="text-xs text-textSecondary">{{ $row['site_name'] }}</p>
                </div>
    ...
    @empty
        <p class="text-sm text-textSecondary">No ledger history yet.</p>
    @endforelse
</div>
