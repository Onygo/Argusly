        ...
        <div class="mt-8 grid gap-6 xl:grid-cols-[1.2fr_0.8fr]">
            <x-dashboard.section title="Topic library" description="Account topics and current-brand topics are shown together for this tenant context.">
                <form method="GET" action="{{ route('app.topics.index') }}" class="mb-5 grid gap-3 md:grid-cols-[1fr_160px_160px_auto]">
                    <input name="search" value="{{ $filters['search'] ?? '' }}" placeholder="Search topics" class="rounded-lg border border-line bg-white px-3 py-2 text-sm text-ink">
                ...
                @else
                    <div class="space-y-3">
                        @foreach ($topics as $topic)
                            <a href="{{ route('app.topics.show', $topic) }}" class="block rounded-lg border border-line bg-panel p-4 transition hover:border-slate-300 hover:bg-white">
                    ...
                    @else
                        <form method="POST" action="{{ route('app.topics.clusters.store') }}" class="space-y-4">
                            ...
                            <label class="block">
                                ...
                                <textarea name="description" rows="3" class="mt-2 w-full rounded-lg border border-line bg-white px-3 py-2 text-sm text-ink"></textarea>
                            </label>
                    ...
                    @else
                        <div class="space-y-3">
                            @foreach ($clusters as $cluster)
                                <a href="{{ route('app.topics.clusters.show', $cluster) }}" class="flex items-center justify-between gap-4 rounded-lg border border-line bg-panel p-4 hover:bg-white">
                                    <div class="min-w-0">
