...
<div class="grid gap-4 sm:grid-cols-2">
    <label class="block sm:col-span-2">
        ...
        <input name="name" value="{{ old('name', $topic->name) }}" required class="mt-2 w-full rounded-lg border border-line bg-white px-3 py-2 text-sm text-ink">
    </label>

    <label class="block">
        <span class="text-xs font-semibold uppercase tracking-[0.1em] text-muted">Slug</span>
        <input name="slug" value="{{ old('slug', $topic->slug) }}" placeholder="Generated from name" class="mt-2 w-full rounded-lg border border-line bg-white px-3 py-2 text-sm text-ink">
    </label>
    ...
    <label class="block sm:col-span-2">
        ...
        <textarea name="description" rows="5" class="mt-2 w-full rounded-lg border border-line bg-white px-3 py-2 text-sm text-ink">{{ old('description', $topic->description) }}</textarea>
    </label>

    @if (current_brand())
        <label class="flex items-start gap-3 rounded-lg border border-line bg-panel p-4">
    ...
    <label class="block">
        ...
        <input name="priority" type="number" min="0" value="{{ old('priority', $topic->brands->first()?->pivot?->priority ?? 0) }}" class="mt-2 w-full rounded-lg border border-line bg-white px-3 py-2 text-sm text-ink">
    </label>

    <label class="block">
        <span class="text-xs font-semibold uppercase tracking-[0.1em] text-muted">Importance score</span>
        <input name="importance_score" type="number" min="0" max="100" step="0.01" value="{{ old('importance_score', $topic->brands->first()?->pivot?->importance_score) }}" class="mt-2 w-full rounded-lg border border-line bg-white px-3 py-2 text-sm text-ink">
    </label>
</div>
