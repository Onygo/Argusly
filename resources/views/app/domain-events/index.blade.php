        ...
        <div class="mt-8">
            <x-dashboard.section title="Recent events" description="Filter product events in the current tenant context.">
                <form method="GET" action="{{ route('app.domain-events') }}" class="mb-5 grid gap-3 md:grid-cols-[1fr_auto]">
                    <select name="event_type" class="rounded-md border border-line bg-white px-3 py-2 text-sm text-ink">
                            ...
                            <article class="rounded-md border border-line bg-panel p-4">
                                <div class="flex flex-col justify-between gap-3 md:flex-row md:items-start">
                                    <div class="min-w-0">
                                        ...
                                        @if ($event->payload)
                                            <pre class="mt-3 max-h-36 overflow-auto rounded-md border border-line bg-white p-3 text-xs leading-5 text-muted">{{ json_encode($event->payload, JSON_PRETTY_PRINT | JSON_UNESCAPED_SLASHES) }}</pre>
                                        @endif
                                    </div>
