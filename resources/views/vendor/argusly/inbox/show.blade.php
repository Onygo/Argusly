...
<head>
    ...
    <title>{{ $draft->title ?: 'Draft' }} - Argusly Inbox</title>
    <script src="https://cdn.tailwindcss.com"></script>
</head>
<body class="bg-slate-50 text-slate-900">
<div class="mx-auto max-w-4xl px-4 py-8">
    <div class="mb-5 flex items-center justify-between">
        <a href="{{ route('argusly-inbox.portal.index', ['site_key' => $siteKey]) }}" class="text-sm font-medium text-blue-700 hover:text-blue-900">Back to Inbox</a>
    ...
    @if ($draft->featured_image_path)
        <img src="{{ \Illuminate\Support\Facades\Storage::disk(config('argusly_connector.images.disk', 'public'))->url($draft->featured_image_path) }}" alt="Featured image" class="mb-6 w-full rounded-lg border border-slate-200 object-cover">
    @endif
    ...
    <div class="mt-6 flex flex-wrap gap-3">
        <form method="POST" action="{{ route('argusly-inbox.portal.approve', ['draft' => $draft->id, 'site_key' => $siteKey]) }}">
            @csrf
        ...
        <form method="POST" action="{{ route('argusly-inbox.portal.publish', ['draft' => $draft->id, 'site_key' => $siteKey]) }}">
            @csrf
