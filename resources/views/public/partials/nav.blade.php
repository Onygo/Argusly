<header class="sticky top-0 z-50 border-b border-border bg-surface/95 backdrop-blur">
    ...
    <div class="mx-auto flex max-w-6xl items-center justify-between px-4 py-3 sm:px-6">
                    ...
                    <div class="absolute left-1/2 top-full z-20 mt-3 w-[22rem] -translate-x-1/2 pl-public-card p-3 shadow-sm">
                        ...
                        <div class="{{ $resourceHubItem !== null ? 'mt-3' : '' }} grid gap-1">
                            @foreach ($resourceItems as $item)
                                ...
                                <a href="{{ MarketingNavigation::buildUrl($item) }}" class="rounded-xl px-3 py-2 text-sm transition-colors {{ $isActive ? 'bg-surfaceMuted text-textPrimary' : 'text-textSecondary hover:bg-surfaceMuted hover:text-textPrimary' }}">
                                    {{ $item['label'] }}
                                </a>
                            @endforeach
                        </div>
        ...
        <div class="flex items-center gap-2">
            <div class="inline-flex items-center gap-1 rounded-md border border-border bg-surface px-2 py-1 text-xs">
                @foreach ($routeSegments->locales() as $switchLocale)
                    @continue(! isset($availableSwitchUrls[$switchLocale]))
                    <a href="{{ $availableSwitchUrls[$switchLocale] }}" class="rounded px-2 py-1 {{ $lang === $switchLocale ? 'bg-publicPrimary text-textInverse' : 'text-textSecondary hover:bg-surfaceMuted' }}">
