<header class="sticky top-0 z-50 border-b border-border bg-surface/95 backdrop-blur">
    ...
    <div class="mx-auto flex max-w-6xl items-center justify-between px-4 py-3 sm:px-6">
        ...
        <div class="flex items-center gap-2">
            <div class="inline-flex items-center gap-2 rounded-full border border-border bg-surface p-1.5 text-sm">
            ...
            <a href="{{ route('login') }}" class="hidden items-center rounded-md px-3 py-2 text-sm text-textSecondary transition-colors hover:bg-surfaceMuted md:inline-flex">
                {{ __('public.nav.sign_in') }}
            </a>
            <a href="{{ MarketingNavigation::buildUrl($primaryCta) }}" class="group hidden items-center justify-center gap-4 rounded-full bg-[#080d16] px-6 py-3 text-base font-semibold text-white transition-colors hover:bg-black md:inline-flex">
                {{ $primaryCta['label'] }}
                <i data-lucide="arrow-right" class="h-5 w-5 transition-transform duration-200 ease-out group-hover:translate-x-0.5" aria-hidden="true"></i>
            </a>
        </div>
    </div>
