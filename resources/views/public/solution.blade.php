<!doctype html>
<html lang="{{ $publicLang ?? app()->getLocale() }}" class="scroll-smooth">
<head>
    <meta charset="UTF-8" />
    <meta name="viewport" content="width=device-width, initial-scale=1" />
    @include('public.partials.seo-head', [
        'metaTitle' => $metaTitle,
        'metaDescription' => $metaDescription,
        'canonicalUrl' => $canonicalUrl,
        'hreflangUrls' => $hreflangUrls ?? [],
        'ogType' => 'website',
    ])
    @include('partials.brand-meta')
    @include('public.partials.argusly-tracking', ['canonicalUrl' => $canonicalUrl])
    @include('public.partials.analytics')
    @vite(['resources/css/app.css', 'resources/js/public.js'])
    <script defer src="https://unpkg.com/lucide@latest"></script>
</head>
<body class="pl-marketing-v2 bg-background text-textSecondary antialiased">
@include('public.partials.analytics-body')
@include('public.partials.nav')

<main class="bg-background" data-page="solution-{{ $solutionKey }}">
    <section class="pl-public-hero">
        <div class="mx-auto grid max-w-6xl gap-10 px-4 py-16 sm:px-6 md:py-20 lg:grid-cols-[minmax(0,1fr)_minmax(340px,0.72fr)] lg:items-center">
            <div>
                <div class="pl-public-hero-label">
                    <i data-lucide="radar" class="h-3.5 w-3.5"></i>
                    {{ $copy['eyebrow'] }}
                </div>
                <h1 class="mt-5 pl-public-heading pl-public-heading-hero">{{ $copy['h1'] }}</h1>
                <p class="mt-5 max-w-3xl text-pretty text-base leading-8 text-textSecondary md:text-lg">{{ $copy['intro'] }}</p>
                <div class="mt-8 flex flex-col gap-3 sm:flex-row">
                    <a href="{{ $contactCta }}" class="pl-public-primary-button">{{ $copy['primary_cta'] }}</a>
                    <a href="{{ $solutionKey === 'ai-visibility' || $solutionKey === 'competitive-intelligence' ? $opportunityUrl : $agenticUrl }}" class="pl-public-secondary-button">{{ $copy['secondary_cta'] }}</a>
                </div>
            </div>

            <div class="pl-public-card p-5">
                <div class="rounded-md border border-border bg-[#fbfaf7] p-4">
                    <div class="flex items-center justify-between border-b border-border pb-3">
                        <span class="pl-public-eyebrow">{{ __('public.solutions.signal_loop') }}</span>
                        <span class="rounded-md bg-publicPrimary px-2.5 py-1 text-xs font-medium text-white">{{ __('public.solutions.live') }}</span>
                    </div>
                    <div class="mt-5 grid gap-3">
                        @foreach ($copy['hero_metrics'] as $metric)
                            <div class="pl-public-card-compact p-4">
                                <p class="text-xs font-semibold uppercase tracking-[0.14em] text-textMuted">{{ $metric['label'] }}</p>
                                <p class="mt-2 text-sm font-semibold text-textPrimary">{{ $metric['value'] }}</p>
                            </div>
                        @endforeach
                    </div>
                </div>
            </div>
        </div>
    </section>

    @include('public.partials.market-links', ['variant' => 'white'])

    <section class="bg-white">
        <div class="mx-auto max-w-6xl px-4 py-16 sm:px-6 md:py-20">
            <div class="grid gap-6">
                @foreach ($copy['sections'] as $index => $section)
                    <article id="{{ \Illuminate\Support\Str::slug($section['eyebrow']) }}" class="grid gap-6 rounded-md border border-border/80 bg-[#fbfaf7] p-6 md:grid-cols-[0.72fr_1.28fr] md:p-8">
                        <div>
                            <p class="pl-public-eyebrow">{{ sprintf('%02d', $index + 1) }} / {{ $section['eyebrow'] }}</p>
                            <h2 class="mt-3 pl-public-heading pl-public-heading-h2">{{ $section['title'] }}</h2>
                        </div>
                        <div>
                            <p class="text-base leading-8 text-textSecondary">{{ $section['text'] }}</p>
                            <ul class="mt-6 grid gap-3 sm:grid-cols-2">
                                @foreach ($section['points'] as $point)
                                    <li class="flex gap-3 rounded-md border border-border bg-white p-4 text-sm leading-6 text-textSecondary">
                                        <x-public.icon name="check" size="xs" class="mt-0.5 flex-none text-publicPrimary" />
                                        <span>{{ $point }}</span>
                                    </li>
                                @endforeach
                            </ul>
                        </div>
                    </article>
                @endforeach
            </div>
        </div>
    </section>

    <section class="pl-public-warm">
        <div class="mx-auto max-w-6xl px-4 py-16 sm:px-6 md:py-20">
            <div class="grid gap-10 lg:grid-cols-[0.72fr_1.28fr]">
                <div>
                    <p class="pl-public-eyebrow">{{ __('public.nav.solutions') }}</p>
                    <h2 class="mt-3 pl-public-heading pl-public-heading-h2">{{ __('public.solutions.related_paths') }}</h2>
                </div>
                <div class="grid gap-4 md:grid-cols-2">
                    @foreach ($allSolutions as $solution)
                        @continue($solution['key'] === $solutionKey)
                        <a href="{{ $solution['url'] }}" class="pl-public-card-compact p-5 transition-colors hover:bg-white">
                            <h3 class="pl-public-heading pl-public-heading-card">{{ $solution['label'] }}</h3>
                            <p class="mt-2 text-sm leading-7">{{ $solution['description'] }}</p>
                        </a>
                    @endforeach
                    <a href="{{ $agenticUrl }}" class="pl-public-card-compact p-5 transition-colors hover:bg-white">
                        <h3 class="pl-public-heading pl-public-heading-card">Agentic Marketing</h3>
                        <p class="mt-2 text-sm leading-7">{{ __('public.solutions.agentic_related') }}</p>
                    </a>
                </div>
            </div>
        </div>
    </section>

    <x-public.faq-section
        page-type="solution"
        :page-slug="$solutionKey"
        :locale="app()->getLocale()"
    />

    <section class="bg-white">
        <div class="mx-auto max-w-6xl px-4 py-16 sm:px-6 md:py-20">
            <div class="pl-public-cta-panel pl-public-cta-panel--split p-8 md:p-10">
                <div class="grid gap-8 lg:grid-cols-[1fr_auto] lg:items-center">
                    <div>
                        <p class="text-sm font-semibold uppercase tracking-[0.14em] text-white/60">{{ $copy['eyebrow'] }}</p>
                        <h2 class="mt-3 max-w-3xl pl-public-heading pl-public-heading-h2 text-white">{{ $copy['cta_title'] }}</h2>
                        <p class="mt-4 max-w-2xl text-sm leading-7 text-white/76">{{ $copy['cta_text'] }}</p>
                    </div>
                    <div class="flex flex-col gap-3 sm:flex-row lg:flex-col">
                        <a href="{{ $contactCta }}" class="pl-public-cta-primary">{{ $copy['primary_cta'] }}</a>
                        <a href="{{ $opportunityUrl }}" class="pl-public-cta-secondary">Opportunity Intelligence</a>
                    </div>
                </div>
            </div>
        </div>
    </section>
</main>

@include('public.partials.footer')

</body>
</html>
