<form method="GET" class="mb-4 grid gap-2 lg:grid-cols-6">
    ...
    <input type="text" name="payment_q" value="{{ $paymentFilters['q'] }}" placeholder="Search payment id or customer ref" class="rounded border border-border bg-background px-2 py-2 text-xs lg:col-span-2">
    <div class="lg:col-span-6 flex gap-2">
        <button class="rounded border border-border px-3 py-1.5 text-xs">Apply filters</button>
        <a href="{{ route('admin.organizations.billing', [$organization, 'tab' => 'payments']) }}" class="rounded border border-border px-3 py-1.5 text-xs text-textSecondary">Reset</a>
    </div>
</form>
                ...
                <div>
                    ...
                    @if ($row['invoice_id'])
                        <p class="text-xs text-textSecondary mt-1">
                            ...
                            <a href="{{ route('admin.invoices.download', $row['invoice_id']) }}" class="underline">download</a>
                        </p>
                    @endif
                </div>
