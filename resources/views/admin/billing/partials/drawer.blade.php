...
<script>
(() => {
    // Simple client-side drawer: rows pass compact JSON payloads in data attributes.
    ...
    const ledgerReferenceRows = (meta) => {
        ...
        return keys
            .filter((key) => meta && meta[key])
            .map((key) => idRow(key, meta[key], true))
            .join('');
    };

    const render = (payload) => {
        const kind = payload.kind || 'details';

        if (kind === 'wallet') {
            title.textContent = 'Wallet details';
            body.innerHTML = `
                <div class="space-y-2">
                    ${idRow('Wallet ID', payload.id, true)}
                    ${idRow('Site', payload.site_name)}
                    ${idRow('Site ID', payload.site_id, true)}
                    ${idRow('Organization', payload.organization_name)}
                    ${idRow('Organization ID', payload.organization_id, true)}
        ...
        if (kind === 'ledger') {
            ...
            body.innerHTML = `
                <div class="space-y-2">
                    ...
                    ${idRow('Reference', payload.reference || '-')}
        ...
        if (kind === 'payment') {
            ...
            const orgUrl = @json(route('admin.organizations.show', $organization));
            const billingUrl = @json(route('admin.organizations.billing', $organization));
            const siteUrl = payload.site_id ? `${billingUrl}?tab=payments&payment_q=${encodeURIComponent(payload.site_id)}` : '';
            body.innerHTML = `
                <div class="space-y-2">
                    ...
                    ${idRow('Subscription/Pack ID', payload.billable_id || '-', Boolean(payload.billable_id))}
                    ${idRow('Billable type', payload.billable_type || '-')}
                    <div class="rounded border border-border px-3 py-2">
                        <p class="text-[11px] uppercase tracking-wide text-textSecondary">Related links</p>
                        <div class="mt-2 flex flex-wrap gap-2 text-xs">
                            <a href="${orgUrl}" class="rounded border border-border px-2 py-1">Organization</a>
                            <a href="${billingUrl}?tab=subscriptions" class="rounded border border-border px-2 py-1">Subscriptions</a>
                            <a href="${billingUrl}?tab=payments" class="rounded border border-border px-2 py-1">Payments</a>
                            ${siteUrl ? `<a href="${siteUrl}" class="rounded border border-border px-2 py-1">Filter by site</a>` : ''}
                            ${payload.checkout_url ? `<a href="${esc(payload.checkout_url)}" target="_blank" rel="noopener" class="rounded border border-border px-2 py-1">Provider dashboard</a>` : ''}
                        </div>
                    </div>
