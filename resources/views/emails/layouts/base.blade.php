...
<head>
    ...
    <title>{{ $subjectLine ?? ($headline ?? 'PublishLayer') }}</title>
</head>
            ...
            <table role="presentation" width="600" cellpadding="0" cellspacing="0" style="width:600px; max-width:600px; background:#ffffff; border:1px solid #e5e7eb; border-radius:6px;">
                <tr>
                    <td style="padding:24px;">
                        <h1 style="margin:0 0 12px; font-size:18px; font-weight:600; line-height:1.4; color:#111827;">{{ $headline ?? 'PublishLayer' }}</h1>
                        ...
                        @if (!empty($cta_url))
                            <p style="margin:18px 0 0;">
                                <a href="{{ $cta_url }}" style="display:inline-block; background:#111827; color:#ffffff; text-decoration:none; border-radius:4px; padding:10px 18px; font-size:14px; font-weight:600;">{{ $cta_label ?? 'Open PublishLayer' }}</a>
                            </p>
                        @endif
                    </td>
                </tr>
                <tr>
                    <td style="padding:16px 24px; border-top:1px solid #e5e7eb;">
                        <p style="margin:0 0 4px; font-size:12px; line-height:1.5; color:#6b7280;">This is a system email from PublishLayer.</p>
