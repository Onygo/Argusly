...
<section class="rounded-lg border border-border bg-surface p-5" aria-label="{{ __('app.dashboard_action_first.active_risks') }}">
    <div class="flex items-start justify-between gap-3">
        ...
        <i data-lucide="shield-alert" class="h-5 w-5 text-amber-600"></i>
    </div>
    <div class="mt-4 space-y-3">
        @forelse (data_get($summary, 'items', collect()) as $risk)
            <a href="{{ route('app.signal-intelligence.detections.show', $risk) }}" class="block rounded-md border border-border bg-background p-3 hover:bg-surfaceMuted">
