<footer class="border-t border-line bg-white">
    <div class="container-page py-14">
        <div class="grid gap-10 md:grid-cols-[1.4fr_repeat(4,1fr)]">
            ...
            @foreach ([
                'Platform' => ['Dashboard', 'Intelligence', 'Visibility', 'Competitors', 'Mentions'],
                'Use cases' => ['Content', 'Campaigns', 'Automations', 'Reports'],
                'Company' => ['About', 'Customers', 'Careers', 'Contact'],
                'Resources' => ['Docs', 'Changelog', 'Blog', 'Security'],
            ] as $heading => $links)
                <div>
                    <h3 class="text-xs font-semibold uppercase tracking-[0.16em] text-muted">{{ $heading }}</h3>
                    <ul class="mt-4 space-y-3 text-sm text-muted">
                        @foreach ($links as $link)
                            <li><a href="#" class="hover:text-ink">{{ $link }}</a></li>
                        @endforeach
                    </ul>
                </div>
            @endforeach
