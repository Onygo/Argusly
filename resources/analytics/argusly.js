...
(function () {
  ...
  function detectArguslyContentId() {
    var metaSelectors = [
      'meta[name="argusly_content_id"]',
      'meta[name="argusly:content_id"]',
      'meta[name="argusly-content-id"]',
      'meta[property="argusly_content_id"]',
      'meta[property="argusly:content_id"]'
    ];
  ...
  var samplingRate = clamp(
    toNumber(window.Argusly.samplingRate || window.Argusly.sampling, 100),
    0,
    100
  );
  ...
  var engagedAfterSeconds = Math.max(
    toNumber(window.Argusly.engagedAfterSeconds || window.Argusly.engaged_after_seconds, DEFAULT_ENGAGED_AFTER_SECONDS),
    1
  );
  var readThroughScrollPercent = clamp(
    toNumber(window.Argusly.readThroughScrollPercent || window.Argusly.read_through_scroll_percent, DEFAULT_READ_THROUGH_SCROLL_PERCENT),
    1,
    100
  );
  var readThroughFallbackSeconds = Math.max(
    toNumber(window.Argusly.readThroughFallbackSeconds || window.Argusly.read_through_fallback_seconds, DEFAULT_READ_THROUGH_FALLBACK_SECONDS),
    1
  );
