...
it('archives published series and hides it from default index', function () use ($makeContext) {
    [$user, $organization, $site] = $makeContext('Series Archive');
    ...
    $this->actingAs($user)
        ->post(route('app.content.series.archive', $publishedSeries))
        ->assertRedirect(route('app.content.series.index'));

    expect((string) $publishedSeries->fresh()->status)->toBe(ContentSeries::STATUS_ARCHIVED);

    $this->actingAs($user)
        ->get(route('app.content.series.index'))
        ->assertOk()
        ->assertDontSee($publishedSeries->name);
    ...
    $this->actingAs($user)
        ->post(route('app.content.series.archive', $draftSeries))
        ->assertForbidden();
});
