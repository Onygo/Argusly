...
return new class extends Migration
{
    public function up(): void
    {
        ...
        Schema::table('argusly_articles', function (Blueprint $table): void {
            if (! Schema::hasColumn('argusly_articles', 'locale')) {
                $table->string('locale', 32)->nullable()->after('featured_image_url')->index();
            }
            if (! Schema::hasColumn('argusly_articles', 'source_locale')) {
                $table->string('source_locale', 32)->nullable()->after('locale');
            }
            if (! Schema::hasColumn('argusly_articles', 'canonical_url')) {
                $table->text('canonical_url')->nullable()->after('source_locale');
            }
            if (! Schema::hasColumn('argusly_articles', 'canonical_content_id')) {
                $table->string('canonical_content_id')->nullable()->after('canonical_url')->index();
            }
            if (! Schema::hasColumn('argusly_articles', 'hreflang_alternates')) {
                $table->json('hreflang_alternates')->nullable()->after('canonical_content_id');
            }
            if (! Schema::hasColumn('argusly_articles', 'x_default_url')) {
                $table->text('x_default_url')->nullable()->after('hreflang_alternates');
            }
            if (! Schema::hasColumn('argusly_articles', 'translation_group_id')) {
                $table->string('translation_group_id')->nullable()->after('x_default_url')->index();
            }
            if (! Schema::hasColumn('argusly_articles', 'family_id')) {
                $table->string('family_id')->nullable()->after('translation_group_id')->index();
            }
            if (! Schema::hasColumn('argusly_articles', 'answer_blocks')) {
                $table->json('answer_blocks')->nullable()->after('family_id');
            }
            if (! Schema::hasColumn('argusly_articles', 'structured_output')) {
                $table->json('structured_output')->nullable()->after('answer_blocks');
            }
            if (! Schema::hasColumn('argusly_articles', 'schema_data')) {
                $table->json('schema_data')->nullable()->after('structured_output');
            }
            if (! Schema::hasColumn('argusly_articles', 'ai_visibility')) {
                $table->json('ai_visibility')->nullable()->after('schema_data');
            }
            if (! Schema::hasColumn('argusly_articles', 'metadata')) {
    ...
    public function down(): void
    {
        ...
        Schema::table('argusly_articles', function (Blueprint $table): void {
            foreach ([
                ...
                if (Schema::hasColumn('argusly_articles', $column)) {
