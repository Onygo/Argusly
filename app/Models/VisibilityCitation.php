<?php

namespace App\Models;

use Illuminate\Database\Eloquent\Attributes\Fillable;
use Illuminate\Database\Eloquent\Factories\HasFactory;
use Illuminate\Database\Eloquent\Model;
use Illuminate\Database\Eloquent\Relations\BelongsTo;
use Illuminate\Support\Str;
use InvalidArgumentException;

#[Fillable([
    'account_id',
    'brand_id',
    'visibility_check_id',
    'provider_run_id',
    'source_url',
    'source_domain',
    'source_title',
    'citation_type',
    'is_owned_source',
    'is_competitor_source',
    'confidence_score',
    'metadata_json',
    'url',
    'domain',
    'title',
    'snippet',
    'rank',
    'trust_score',
    'metadata',
])]
class VisibilityCitation extends Model
{
    use HasFactory;

    protected static function booted(): void
    {
        static::creating(function (VisibilityCitation $citation): void {
            $citation->uuid ??= (string) Str::uuid();
        });

        static::saving(function (VisibilityCitation $citation): void {
            $citation->url ??= $citation->source_url;
            $citation->domain ??= $citation->source_domain;
            $citation->title ??= $citation->source_title;
            $citation->source_url ??= $citation->url;
            $citation->source_domain ??= $citation->domain;
            $citation->source_title ??= $citation->title;
            $citation->confidence_score ??= $citation->trust_score;
            $citation->metadata_json ??= $citation->metadata;

            if ($citation->trust_score !== null && ($citation->trust_score < 0 || $citation->trust_score > 100)) {
                throw new InvalidArgumentException('Visibility citation trust score must be between 0 and 100.');
            }

            if ($citation->confidence_score !== null && ($citation->confidence_score < 0 || $citation->confidence_score > 100)) {
                throw new InvalidArgumentException('Visibility citation confidence score must be between 0 and 100.');
            }

            $run = VisibilityProviderRun::query()->find($citation->provider_run_id);

            if (! $run || $run->account_id !== $citation->account_id || $run->brand_id !== $citation->brand_id) {
                throw new InvalidArgumentException('Visibility citation provider run must belong to the same tenant.');
            }

            if ($citation->visibility_check_id !== null) {
                $check = VisibilityCheck::query()->find($citation->visibility_check_id);

                if (! $check || $check->account_id !== $citation->account_id || $check->brand_id !== $citation->brand_id) {
                    throw new InvalidArgumentException('Visibility citation check must belong to the same tenant.');
                }
            }
        });
    }

    /**
     * @return BelongsTo<VisibilityProviderRun, $this>
     */
    public function providerRun(): BelongsTo
    {
        return $this->belongsTo(VisibilityProviderRun::class, 'provider_run_id');
    }

    protected function casts(): array
    {
        return [
            'rank' => 'integer',
            'trust_score' => 'integer',
            'is_owned_source' => 'boolean',
            'is_competitor_source' => 'boolean',
            'confidence_score' => 'integer',
            'metadata' => 'array',
            'metadata_json' => 'array',
        ];
    }
}
