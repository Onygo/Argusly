...
class CalculateBasicPageScoresJob implements ShouldQueue
{
    use Dispatchable;
    ...
    {
        $this->onQueue((string) config('page_intelligence.queues.score', 'page_intelligence_score'));
    }
    ...
    public function handle(PageAnalysisService $analysis): void
    {
        ...
        $analysis->calculateBasicScores($snapshot);
    }

    public function middleware(): array
    {
        return [(new WithoutOverlapping('page-intelligence:scores:'.$this->pageSnapshotId))->releaseAfter(60)->expireAfter(600), new PageIntelligenceHostRateLimit];
    }
