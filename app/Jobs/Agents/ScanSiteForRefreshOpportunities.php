...
class ScanSiteForRefreshOpportunities implements ShouldBeUnique, ShouldQueue
{
    ...
    public function handle(
        ...
        if (! $settingsResolver->automaticRecommendationGenerationEnabledForSite($site)) {
            return;
        }
        ...
        foreach ($contents as $content) {
            $runner->runForContent(
                agent: $agent,
                content: $content,
                triggerSource: 'schedule.site_refresh_scan',
                metadata: [
                    'scan_type' => 'refresh',
