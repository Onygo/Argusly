...
class ArguslyTrackingSiteResolver
{
    ...
    public function getTrackingScriptUrl(): string
    {
        ...
        $version = (string) config('argusly.tracking_script_version', '1.1.0');

        return $baseUrl . '/argusly.js?v=' . rawurlencode($version);
    }
    ...
    public function shouldInjectTracking(): bool
    {
        ...
        if (app()->environment('testing') && ! config('argusly.analytics.allow_tracking_in_testing', false)) {
            return false;
        }

        if (app()->environment('local') && ! config('argusly.analytics.allow_tracking_on_local', false)) {
            return false;
        }
