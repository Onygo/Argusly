...
class ArguslyConnectorBlogSource implements ConnectorPublicBlogSource
{
    ...
    {
        return (bool) config('argusly_connector.public_blog.use_connector', config('argusly.public_blog.use_connector', true));
    }
    ...
    {
        return (string) config('argusly_connector.public_blog.connector_endpoint', config('argusly.public_blog.connector_endpoint', '/v1/public/blog/posts'));
    }
    ...
    {
        $connection = (array) config('argusly_connector.connections.default', []);
        $apiConfig = (array) config('argusly_connector.api', []);
        $httpConfig = (array) config('argusly_connector.http', []);
        $baseUrl = (string) ($connection['base_url'] ?? ($apiConfig['base_url'] ?? config('argusly_connector.base_url', 'https://api.argusly.com')));
        ...
        $apiKey = trim((string) ($connection['api_key'] ?? ($apiConfig['api_key'] ?? config('argusly_connector.api_key', ''))));
        if ($apiKey !== '') {
            $request = $request->withToken($apiKey);
        }

        $workspaceId = trim((string) ($connection['workspace_id'] ?? ($apiConfig['workspace_id'] ?? config('argusly_connector.workspace_id', ''))));
        if ($workspaceId !== '') {
            $request = $request->withHeaders([
                'X-Argusly-Workspace' => $workspaceId,
                'X-Argusly-Workspace' => $workspaceId,
            ]);
        }
    ...
    {
        $flagEnabled = (bool) config('argusly_connector.http_insecure_local', false);
        if (! $flagEnabled) {
            return [];
        }

        if ($this->isProductionEnvironment()) {
            throw new RuntimeException('ARGUSLY_HTTP_INSECURE_LOCAL cannot be enabled in production.');
        }
