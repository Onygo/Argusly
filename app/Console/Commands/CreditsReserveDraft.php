...
class CreditsReserveDraft extends Command
{
    protected $signature = 'credits:reserve-draft {draft_id} {--user_id=}';
    protected $description = 'Reserve credits for a draft (idempotent).';
    ...
    public function handle(CreditWalletService $credits): int
    {
        ...
        $credits->ensureReservedForDraft($draft, $userId);

        $draft->refresh();

        $this->info('Draft reserved.');
        $this->table(
